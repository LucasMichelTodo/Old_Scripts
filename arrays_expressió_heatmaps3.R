...
## Create eSet
exprsx <- as.matrix(df[,c(-1,-2,-9,-10,-11)])
fdata <- new("AnnotatedDataFrame",df[,c(1:2,9:11)])
time <- factor(c(1,1,2,2,3,3))
soca <- factor(rep(c("Ind", "Ctl"),3))
pdata <- data.frame(soca=soca,time=time); rownames(pdata) <- colnames(df[,c(-1,-2,-9,-10,-11)])
