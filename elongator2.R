  ...
  if (i < length(df$chrom)){ #Avoid breaking in last row
    if (df[i,"chrom"] == df[i+1,"chrom"]){ #Avoid comparing two different chromosomes when chromosome changes.
      ...
      if ( dif < 2000 && dif > 0){
        ...
        df[i,"stop"] <- df[i,"stop"] + 2000
        df[i+1, "start"] <- df[i+1, "start"] -2000
        df[i,"post"] <- 2000
        df[i+1,"pre"] <- 2000
      }
      chrom <- df[]
    } else { #Handle rows where there is a chromosome change (last row of a chrom)
      if (df[i+1,"start"] > 2000) { #Handle start of next chromosome
        df[i+1,"start"] <- df[i+1,"start"]-2000
        df[i+1, "pre"] <- 2000
      } else {
      ...
      if (df[i,"stop"] + 2000 > chrend) { #Handle end of current chromosome
    ...
    chrend <- chrom_lengths[chrom_lengths$V1 %in% df[i,"chrom"],2]
    if (df[i,"stop"] + 2000 > chrend) { #Handle last row 
      df[i,"post"] <- chrend - df[i,"stop"]
      df[i,"stop"] <- chrend
    } else {
      df[i,"stop"] <- df[i,"stop"] + 2000
...
write.table(df, file = "/home/lucas/ISGlobal/Gen_Referencies/Elongated_genes_4.gff", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
head(df, 50)
