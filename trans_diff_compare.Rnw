...
diff_10G_met <- read.table("/home/lucas/ISGlobal/metil_diff_10G_12B_q5.txt", fill = TRUE, fileEncoding = "UTF-8")
#diff_10G_met <- gsub("ID=", "", diff_10G_met[,1], fixed = TRUE)

# Differential peak calling 1.2B
diff_12B_met <- read.table("/home/lucas/ISGlobal/metil_diff_12B_10G_q5.txt", fill = TRUE, fileEncoding = "UTF-8")
diff_12B_met <- gsub("ID=", "", diff_12B_met[,1], fixed = TRUE)

#Comprovant que tots surten al diccionari
diff_10G_met[,1] %in% rosetta[,1]
...
# Taula
table(diff_trans[,2] %in% diff_10G_met[,1] | diff_trans[,2] %in% diff_12B_met)

# Gens que són a la llista de diff_peaks
diff_trans[diff_trans[,2] %in% diff_10G_met[,1] | diff_trans[,2] %in% diff_12B_met,]
