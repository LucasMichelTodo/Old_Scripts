...
eset_10E_HS <- xgene_impute[,xgene_impute@phenoData@data$subclone == "s10E" & xgene_impute@phenoData@data$status == "HS"]
eset_10G_HS <- xgene_impute[,xgene_impute@phenoData@data$subclone == "s10G" & xgene_impute@phenoData@data$status == "HS"]


df_DIF_10Evs10G <- exprs(eset_10E_HS)-exprs(eset_10G_HS)
df_DIF_10Evs10G <- data.frame(df_DIF_10Evs10G)

df_DIF_10Evs10G$Max_Dif <- as.numeric(apply(df_DIF_10Evs10G, 1, function(x) x[which.max(abs(x))]))

df_DIF_10Evs10G$Gene <- rownames(df_DIF_10Evs10G)
imputed_10Evs10G_DIFS <- arrange(df_DIF_10Evs10G, -abs(Max_Dif))

# eset_10E <- xgene[,xgene@phenoData@data$subclone == "s10E"]
# 
# df_10E_CTL <- exprs(eset_10E)[,eset_10E@phenoData@data$status == "CTL"]
# df_10E_HS <- exprs(eset_10E)[,eset_10E@phenoData@data$status == "HS"]
# 
...
# 
# df_DIF_10E$Gene <- rownames(df_DIF_10E)
# base_10E_difs <- arrange(df_DIF_10E, -abs(Max_Dif))
# 
# compare_df <- merge(imputed_10E_difs, base_10E_difs, by = "Gene")
# corrected <- arrange(compare_df, Max_Dif.x-Max_Dif.y)[1:10,]$Gene
...
for (i in 1:n_genes_toprint){
  ...
  s10E <- xgene_impute[,xgene_impute@phenoData@data$subclone == "s10E"]
  ...
  p <- p + geom_point(aes(color = Type, shape = Type)) + geom_line() + scale_linetype_manual(values=c("dashed", "solid"))
  ...
  #ggsave(p, file=paste0(figuresPath,"Imputed/", formatC(i, width=nchar(n_genes_toprint), flag="0"), "_", gene, ".jpeg"), device = "jpeg", width = 14, height = 10, units = "cm")
  print(p)
}
