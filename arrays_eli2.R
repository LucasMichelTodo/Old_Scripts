...
## Create eSet
exprsx <- as.matrix(df[,c(-1,-2,-27,-28,-29)])
fdata <- new("AnnotatedDataFrame",df[,c(1:2,27:29)])
time <- factor(rep(c(0,0,1,1,2,2,3,3), 3))
soca <- factor(gsub("(_.*_)", "_",colnames(exprsx), perl = TRUE))
pdata <- data.frame(soca=soca,time=time); rownames(pdata) <- colnames(df[,c(-1,-2,-27,-28,-29)])
...
# Ara només en fa 10
for (i in 1:10){
  fileName <- paste("/home/lucas/ISGlobal/Arrays/Eli_Arrays/",gsub('/','',gsub(':','#',featureNames(x)[i])),'.png',sep='') #colon is not supported under windows
  png(fileName)
  cat(paste(i,'\n'))
  plot.new()
  par(las=2)
...
sel_all <- filter(heatmap_df, abs(T2) >  0.4771213 | abs(T3) >  0.4771213 | abs(T4) >  0.4771213)
dim(sel_all)
10**0.39794
log10(3)
...
for (i in 1:10){
  ...
  ggsave(p, file=paste(custom_plots_dir,gsub('/','',gsub(':','#',featureNames(xgene)[i])),'.png',sep=''), device = "png", width = 14, height = 10, units = "cm")
}
