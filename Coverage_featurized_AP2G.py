...
nreads = {}
#with open("/home/lucas/ISGlobal/Chip_Seq/Noves_dades/chip_nou_nreads.txt", "r+") as infile:  ## Old Chip
with open("/home/lucas/ISGlobal/Chip_Seq/DATA/Aligns/q5/chip_vell_nreads.txt", "r+") as infile: ## New Chip
	for line in infile:
...
def featurized_coverage(bamfile):
	...
	with open("/home/lucas/ISGlobal/Gen_Referencies/AP2G.gff", "r") as file:
		...
		for line in gff_file:
			pre = get_coverage(bamfile, line[0], int(line[3]), int(line[3])+int(line[9]))[0] / nreads[bamfile]*1000000.0
			post = get_coverage(bamfile, line[0], int(line[4])-int(line[10]), int(line[4]))[0] / nreads[bamfile]*1000000.0
			covGene = get_coverage(bamfile, line[0], int(line[3])+int(line[9]), int(line[4])-int(line[10]))[0] / nreads[bamfile]*1000000.0
			...
			with open(bamfile.replace("_sort_q5.bam", "_cov_vector_pre_AP2G.csv"), "a+") as file3:
				vect = (get_coverage(bamfile, line[0], int(line[3]), int(line[3])+int(line[9]))[1] / nreads[bamfile])*1000000.0
				for element in vect:
					file3.write(str(element)+",")

			with open(bamfile.replace("_sort_q5.bam", "_cov_vector_post_AP2G.csv"), "a+") as file3:
				vect = (get_coverage(bamfile, line[0], int(line[4])-int(line[10]), int(line[4]))[1] / nreads[bamfile])*1000000.0
				for element in vect:
					file3.write(str(element)+",")

			with open(bamfile.replace("_sort_q5.bam", "_cov_vector_ORF_AP2G.csv"), "a+") as file3:
				vect = (get_coverage(bamfile, line[0], int(line[3])+int(line[9]), int(line[4])-int(line[10]))[1] / nreads[bamfile])*1000000.0
				for element in vect:
					file3.write(str(element)+",")
