...
def diff_peak_call(bams):
	...
	subprocess.call("mkdir macs2_results_c20", shell=True)
	...
	for name in names:  #Callpeaks for each alignment vs it's input control.
		cmd = "macs2 callpeak -g 2.3e+7 -f BAMPE -B -t {}_me_sort.bam -c {}_in_sort.bam -n {} --nomodel --extsize 117 --outdir ./macs2_results_c20" .format(name, name, name[2:])
		subprocess.call(cmd, shell=True)
		
		cmd = "egrep \"fragments after filtering in treatment|fragments after filtering in control\" ./macs2_results_c20/{}_peaks.xls" .format(name[2:]) #Get sequencing depth of the alignment.
	...
	for pair in pairs:
		...
		cmd = "macs2 bdgdiff --t1 ./macs2_results_c20/{}_treat_pileup.bdg --c1 ./macs2_results_c20/{}_control_lambda.bdg --t2 ./macs2_results_c20/{}_treat_pileup.bdg --c2 ./macs2_results_c20/{}_control_lambda.bdg --d1 {} --d2 {} -C 20 -g 150 -l 200 --o-prefix {} --outdir ./macs2_results_c20" \
