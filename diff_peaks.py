...
def diff_peak_call(bams):
	...
	subprocess.call("mkdir macs2_results", shell=True)
	...
	for name in names:  #Callpeaks for each alignment vs it's input control.
		cmd = "macs2 callpeak -g 2.3e+7 -f BAMPE -B -t {}_me_sort.bam -c {}_in_sort.bam -n {} --nomodel --extsize 117 --outdir ./macs2_results" .format(name, name, name[2:])
		subprocess.call(cmd, shell=True)
		
		cmd = "egrep \"fragments after filtering in treatment|fragments after filtering in control\" ./macs2_results/{}_peaks.xls" .format(name[2:]) #Get sequencing depth of the alignment.
	...
	for pair in pairs:
		...
		cmd = "macs2 bdgdiff --t1 ./macs2_results/{}_treat_pileup.bdg --c1 ./macs2_results/{}_control_lambda.bdg --t2 ./macs2_results/{}_treat_pileup.bdg --c2 ./macs2_results/{}_control_lambda.bdg --d1 {} --d2 {} -g 100 -l 117 --o-prefix {} --outdir ./macs2_results" \
