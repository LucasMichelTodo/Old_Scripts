...
varifin <- c("PF3D7_0711700", "PF3D7_1300200", "PF3D7_0808900", "PF3D7_0100200", "PF3D7_1100500", "PF3D7_0221900", "PF3D7_1100300", "PF3D7_0425900")
cov["varifin"] <- FALSE
cov_12B["varifin"] <- FALSE
varifin_df <- ref[ref$ID %in% varifin,]
for (i in 1:dim(varifin_df)[1]){
  start <- varifin_df[i,4]
  stop <- varifin_df[i,5]
  chr <- varifin_df[i,1]
  cov[cov$Chom == chr,][which.min(abs(cov[cov$Chom == chr,2] - start)): which.min(abs(cov[cov$Chom == chr,2] - stop)),"varifin"] <- TRUE
  cov_12B[cov_12B$Chom == chr,][which.min(abs(cov_12B[cov_12B$Chom == chr,2] - start)): which.min(abs(cov_12B[cov_12B$Chom == chr,2] - stop)),"varifin"] <- TRUE
  }
@
...
<<trans_histo, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE>>=
ggplot(trans_df, aes(log(trans_df$Aver.2Higher10G.))) + geom_histogram(bins = 200)
@
\clearpage
<<met_histo, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE>>=
...
En un primer intent de diferenciar variants actius d'inactius hem creat el paremetre "silenced" que hem considerat "TRUE" si el valor de metilació del fragment està per sobre d'un llindar ($>$3). (Els valors de metilació utilitzats corresponen a l'ORF de cada gen i s'han expandit a tots els fragments que en formen part.)
 <<ac_met_log_status, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE, fig.pos = "b">>=
cov["Class"] <- "Regular"
cov[cov$Variant == TRUE & cov$noexprs == TRUE,]$Class <- "Variant-Silenced"
cov[cov$Variant == TRUE & cov$noexprs == FALSE,]$Class <- "Variant-Active"
table(cov$Class)
df <- cov[cov$Class != "Regular",]
df <- df[df$Type == "ORF",]
...
<<ac_met_log, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE, fig.pos = "b">>=
...
<<ac_met_log_status_10G_12B_ORF, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE, fig.pos = "b">>=
df <- cov[cov$Type == "ORF",]
df2 <- cov_12B[cov_12B$Type == "ORF",]
df["Class"] <- "Regular"
df2["Class"] <- "Regular"
...
<<ac_met_log_status_10G_12B_5, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE, fig.pos = "b">>=
df <- cov[cov$Type == "5prima",]
df2 <- cov_12B[cov_12B$Type == "5prima",]
df["Class"] <- "Regular"
df2["Class"] <- "Regular"
df[df$Variant == TRUE,]$Class <- "Variant"
df2[df2$Variant == TRUE,]$Class <- "Variant"
...
df <- rbind(sample_n(df[df$Class == "Regular",], 1000), 
            sample_n(df[df$Class == "Variant",], 1000), 
            df[df$Class == "Variant-Active",],
            df[df$Class == "Variant-Inactive",])
...
<<ac_met_log_status_10G_12B_3, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE, fig.pos = "b">>=
df <- cov[cov$Type == "3prima",]
df2 <- cov_12B[cov_12B$Type == "3prima",]
df["Class"] <- "Regular"
df2["Class"] <- "Regular"
df[df$Variant == TRUE,]$Class <- "Variant"
df2[df2$Variant == TRUE,]$Class <- "Variant"
...
<<ac_met_log_status_10G_12B_varifin, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE, fig.pos = "b">>=
df <- cov
df2 <- cov_12B
df["Class"] <- "Regular"
df2["Class"] <- "Regular"
df[df$Variant == TRUE,]$Class <- "Variant"
df2[df2$Variant == TRUE,]$Class <- "Variant"
df[df$difexp != "FALSE" & df$varifin == TRUE & df$Variant == TRUE & df$difexp =="UP",]$Class <- "Variant-Active"
df[df$difexp != "FALSE" & df$varifin == TRUE & df$Variant == TRUE & df$difexp =="DOWN",]$Class <- "Variant-Inactive"
df2[df2$difexp != "FALSE" & df2$varifin == TRUE & df2$Variant == TRUE & df2$difexp =="UP",]$Class <- "Variant-Active"
df2[df2$difexp != "FALSE" & df2$varifin == TRUE & df2$Variant == TRUE & df2$difexp =="DOWN",]$Class <- "Variant-Inactive"
...
<<ac_met_log_status_10G_12B_novarifin, echo=FALSE, warning=FALSE, out.width="1\\linewidth", message=FALSE, fig.pos = "b">>=
df <- cov
df2 <- cov_12B
df["Class"] <- "Regular"
df2["Class"] <- "Regular"
df[df$Variant == TRUE,]$Class <- "Variant"
df2[df2$Variant == TRUE,]$Class <- "Variant"
df[df$difexp != "FALSE" & df$varifin == FALSE & df$Variant == TRUE & df$difexp =="UP",]$Class <- "Variant-Active"
df[df$difexp != "FALSE" & df$varifin == FALSE & df$Variant == TRUE & df$difexp =="DOWN",]$Class <- "Variant-Inactive"
df2[df2$difexp != "FALSE" & df2$varifin == FALSE & df2$Variant == TRUE & df2$difexp =="UP",]$Class <- "Variant-Active"
df2[df2$difexp != "FALSE" & df2$varifin == FALSE & df2$Variant == TRUE & df2$difexp =="DOWN",]$Class <- "Variant-Inactive"
...
test_df["all_0.5"] <- sample(c(0,1), replace=TRUE, size=8663)
