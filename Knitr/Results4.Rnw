...
for (i in 1:length(rownames(trans_percent))){
  print(gsub("description=", "", strsplit(as.character(cov_10G[cov_10G$Gene == rownames(trans_percent)[i],]$Annotations[1]), split = ";")[[1]][2]), fixed = TRUE)
  rownames(trans_percent)[i] <- paste(rownames(trans_percent)[i], gsub("description=", "", strsplit(as.character(cov_10G[cov_10G$Gene == rownames(trans_percent)[i],]$Annotations[1]), split = ";")[[1]][2]), sep = "-->")
}
rownames(heatmap_df) <- rownames(trans_percent)
...
diff <- data.frame(cov_1.2B$Gene, diff_5, diff_ORF, diff_3)
...
cor(heatmap_df$Trans, heatmap_df$Met, method = "pearson")
cor(heatmap_df$Trans, heatmap_df$Met, method = "spearman")
...
<<correlacions_met, echo=FALSE, warning=FALSE>>=
########### Filtrats per Metilació ###################
