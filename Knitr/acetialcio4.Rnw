...
cov_met_1.2B <- read.table("/home/lucas/ISGlobal/Chip_Seq/DATA/Aligns/q5/10G_me_sort_q5_fullcoverage.bed", header = FALSE,  sep ="\t", quote = "", row.names = NULL, stringsAsFactors = FALSE)
cov_ac <- read.table("/home/lucas/ISGlobal/Chip_Seq/DATA/Aligns/q5/10G_ac_sort_q5_fullcoverage.bed", header = FALSE,  sep ="\t", quote = "", row.names = NULL, stringsAsFactors = FALSE)
...
trans_df2 <- trans_df[,c(1,25,26)]
trans_df2["Trans_dif"] <- trans_df2[,3] - trans_df2[,2]
trans_maxdif <- arrange(trans_df2, -abs(trans_df2$Trans_dif))[1:30,]
...
noexprs <- trans_df[log(trans_df$Aver.2Higher10G.) < 6,]$ID
...
df <- cov[cov$difexp != "FALSE" & cov$Type == "ORF",]
table(df$Variant)
df["Class"] <- "Regular"
df[df$Variant == TRUE & df$difexp =="UP",]$Class <- "Variant-Active"
df[df$Variant == TRUE & df$difexp =="DOWN",]$Class <- "Variant-Inactive"
ggplot(df, aes(x = df$Met, y = df$Ac, color = df$Class)) +
  scale_x_log10() + scale_y_log10() +
  geom_point(size=0.2)
@
