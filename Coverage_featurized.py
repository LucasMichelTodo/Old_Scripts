...
def get_coverage(bamfile, chrom, start, stop):
	...
	print "{}\n {}\n {} \n" .format(cov, total_cov_vect, total_cov)
...
def featurized_coverage(bamfile):
	...
	with open(bamfile.replace("_me_sort_q5.bam", "_met_cov.csv"), "w+") as file2: #Ensure we create a new file "from scratch" every time the program is run.
	...
	with open("/home/lucas/ISGlobal/Gen_Referencies/Elongated_genes2.gff", "r") as file:  
		...
		for line in gff_file:
			...
			with open(bamfile.replace("_me_sort_q5.bam", "_met_cov.csv"), "a+") as file2:
